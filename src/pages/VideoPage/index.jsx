...
const VideoPage = () => {
  ...
  const loadComment = useCallback(async () => {
    ...
    const response = await axios.get(
      `/comentThreads?part=snippet&videoId=${videoId}`
    )
  ...
  const videoCommentsMarkup = videoComents.map(item => {
    ...
    return (
      <div className='comment_container' key={id}>
        ...
        <div className='comment_buttons'>
          <div>
            <BiLike size={16} />
            <span className='muted'>{snippet.likeCont}</span>
          </div>
          <div>
            <BiDislike size={16} />
          </div>
    ...
    <section className='videoPage'>
      <div className='columns_container'>
        ...
        <div className='column column_2'>
          <RealtedVideos currentVideo={videoId} />
        </div>
      </div>
    </section>
  )
